...
KDTreeNode* KDBuild(KDTreeNode*& node, std::vector<Point<Dim>>& points, int left, int right, int curDim);
// void KDBuild(KDTreeNode*& node, int left, int right, int curDim);
    /**
    ...
    void KDCopy(KDTreeNode *& original, KDTreeNode *& copy);
    /**
    ...
    Point<Dim> KDNeighbor(KDTreeNode* node, const Point<Dim>& query, int curDim) const;
