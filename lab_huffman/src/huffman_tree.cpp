                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     * the queues. Return this item after removing it from itsingledqueue.
     */
    ...
    if (singleQueue.empty()) {
        TreeNode* Merged = mergeQueue.front();
        mergeQueue.pop();
        return Merged;
    }
    if (mergeQueue.empty()) {
        TreeNode* singled= singleQueue.front();
        singleQueue.pop();
        return singled;
    }
    TreeNode* singled= singleQueue.front();
    TreeNode* Merged = mergeQueue.front();
    if (singled->freq < Merged->freq) {
        singleQueue.pop();
        return singled;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        TreeNode* parent = new TreeNode(t1->freq.getFrequency() + t2->freq.getFrequency()); 
        parent->left = t1;
        parent->right = t2;
