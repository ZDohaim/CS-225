...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    // for(size_t i= 0; i < first_larger_idx; i++){
    //     if(first_larger_idx == subroot->elements[i]){ 
    //         return (subroot->elements[i]).value;
    //     }
    // }
...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    //insert(BTreeNode* subroot, const DataPair& pair)
    parent->elements.insert(elem_itr, *mid_elem_itr);
    parent->children.insert(child_itr, new_right);

    // vector: assign (size_type n, const value_type& val)
    ...
    //     }
        ...
        if (subroot->children[first_larger_idx]->elements.size() >= order) {
            ...
            if (first_larger_idx < subroot->elements.size() && pair.key > subroot->elements[first_larger_idx].key) {
                insert(subroot->children[first_larger_idx + 1], pair);
            }
        }
    }
}
