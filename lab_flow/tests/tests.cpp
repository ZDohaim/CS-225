...
vector<int> getRandomHelper(unsigned n, unsigned upper) {
    static std::mt19937 rng; // only want to create rng once

    vector<int> v;
    for (unsigned i = 0; i < n; i++) {
        v.push_back(rng() % upper);
    }
    return v;
}
...
TEST_CASE("Graph a - rand -> b  has flow of rand.", "[valgrind][weight=1]") {
  ...
  vector<int> r = getRandomHelper(1,1000);
  g.insertEdge("a","b");
...
TEST_CASE("Testing random flow through two pipes", "[valgrind][weight=1]") {
  ...
  vector<int> r = getRandomHelper(3,1000);
  int result = (r[0] < r[1]) ? r[0] + r[2] : r[1] + r[2];
  g.insertEdge("a","b");
  g.setEdgeWeight("a","b",r[0]);
  g.insertEdge("b","c");
...
TEST_CASE("Testing random backflow", "[valgrind][weight=1]") {
  ...
  vector<int> r = getRandomHelper(5, 1000);
  int result = ((r[0] + r[1]) < (r[2] + r[3])) ? r[0] + r[1] : r[2] + r[3];
